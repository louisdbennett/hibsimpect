...
get_events_from_comps = function(token, compids){
  ...
  for(l in 1:length(compids)){
    ...
    for (k in 1:length(matchids)){
      ...
      print(paste(l,"/",length(compids),"comps. ",k,"/",length(matchids),"matches. status: ",test$status_code))
      if (test$status_code == 200) {
        ...
        print(paste(l,"/",length(compids),"comps. ",k,"/",length(matchids),"matches. status: ",eventData$status_code))
      }
      ...
      if (test$status_code == 200) {
        ...
        Sys.sleep(1)
        t <- t+1
        matchDetails <- content(GET(url = paste0("https://api.impect.com/v4/customerapi/matches/",matchID), add_headers(Authorization = paste("Bearer", token, sep = " "))))
      }
      if(length(eventData$data$events)==0) next
      ...
      kpis[is.na(kpis)] <- 0
      events <- cbind(matchid,events,kpis)

      if (exists("matchEvents"))  {
        matchEvents <- rbind(matchEvents,events)
      } else {
        matchEvents <- events
      }
    }
