...
get_matches = function(token, comp){
  ...
  matches = setNames(data.frame(matrix(ncol = 14, nrow = 0)),
                     c("match_id","match_date","kick_off","home_team_id","home_team","away_team_id","away_team","gameweek_id","gameweek","comp_id","competition","season_id","season","score"))

  for (match_id in matchids){
    test <- (GET(url = paste0("https://api.impect.com/v4/customerapi/matches/",match_id), add_headers(Authorization = paste("Bearer", token, sep = " "))))
    ...
    matchdetails <- data.frame(match_id,match_date,kick_off,home_team_id,home_team,away_team_id,away_team,gameweek_id,gameweek,comp_id,competition,season_id,season,score)

    matches <- rbind(matches, matchdetails)
  }
